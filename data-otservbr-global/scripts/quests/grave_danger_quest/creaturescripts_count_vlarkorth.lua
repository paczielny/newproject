...
function count_vlarkorth_transform.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType)  
	-- Configurar cooldowns para jugadores  
	...
	-- Inicializar storage de daño si no existe  
	...
	-- Verificar inmunidad por invocaciones activas  
	if creature:getStorageValue(3) > 0 then  
		creature:getPosition():sendMagicEffect(CONST_ME_BLOCKHIT)  
		return primaryDamage, primaryType, secondaryDamage, secondaryType  
	end  
  
	-- SIEMPRE acumular daño, independientemente del atacante  
	local totalDamage = primaryDamage + secondaryDamage  
	creature:setStorageValue(1, damageStorage + totalDamage)  
  
	-- Verificar si debe activar la mecánica  
	local health = creature:getMaxHealth() * 0.15  
	if creature:getStorageValue(1) >= health then  
		creature:setStorageValue(1, 0) -- Reset damage counter  
		creature:setStorageValue(3, 0) -- Reset summon counter  
		summonDarks()  
	end  
